...
MariaDBDocumentStore.prototype.createTable = function () {
  ...
  this.pool.query(createTableQuery, function (err) {
    if (err) {
      ...
      winston.info('MariaDB table "entries" created or already exists');
    }
  });
};
...
MariaDBDocumentStore.prototype.set = function (key, data, callback, skipExpire) {
  ...
  this.pool.query(
    'INSERT INTO entries (`key`, value, expiration) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE value = VALUES(value), expiration = VALUES(expiration)',
    [key, data, expiration],
...
MariaDBDocumentStore.prototype.get = function (key, callback, skipExpire) {
  ...
  this.pool.query(
    'SELECT value, expiration FROM entries WHERE `key` = ? AND (expiration IS NULL OR expiration > ?)',
    [key, now],
    function (err, results) {
      if (err) {
      ...
      if (results.length) {
        ...
        if (result.expiration && !skipExpire) {
          this.pool.query(
            'UPDATE entries SET expiration = ? WHERE `key` = ?',
